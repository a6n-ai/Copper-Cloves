...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const booking = await prisma.booking.findUnique({
    ...
    include: {
      class_schedule: {
        select: { instructor_id: true, start_time: true, instructor_check_in_time: true },
      },
    },
  });
  ...
  // Instructor must check in before marking members
  if (!booking.class_schedule?.instructor_check_in_time)
    return res.status(400).json({
      error: "You must check in first before marking member attendance.",
      code: "INSTRUCTOR_NOT_CHECKED_IN",
    });
