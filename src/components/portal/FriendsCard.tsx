...
function Avatar({ name, avatarUrl }: { name: string; avatarUrl: string | null }) {
  ...
  return <div className="w-9 h-9 rounded-full bg-sage text-cream flex items-center justify-center text-xs font-semibold">{initials}</div>;
}
...
export function FriendsCard() {
  ...
  return (
    <Card className="border-border bg-card">
      <CardHeader>
        <CardTitle className="font-body text-xl text-charcoal">Friends</CardTitle>
      </CardHeader>
      <CardContent className="space-y-5">
        {requests.incoming.length > 0 && (
          <div className="space-y-2">
            <p className="text-xs uppercase tracking-wide text-muted-foreground">Friend requests</p>
        ...
        {requests.outgoing.length > 0 && (
          <div className="space-y-2">
            <p className="text-xs uppercase tracking-wide text-muted-foreground">Sent requests</p>
        ...
        <div className="space-y-2">
          {friends.length === 0 ? (
            <p className="text-sm text-muted-foreground">Invite friends to a class and they&apos;ll show up here.</p>
          ) : (
        ...
        {suggestions.length > 0 && (
          <div className="space-y-2">
            <p className="text-xs uppercase tracking-wide text-muted-foreground">People you may know</p>
            <div className="flex flex-wrap gap-2">
              {suggestions.map((s) => (
                <div key={s.id} className="flex items-center gap-2 border border-border rounded-lg px-2.5 py-1.5">
                  <Avatar name={s.name} avatarUrl={s.avatar_url} />
                  <div className="min-w-0">
                    <div className="text-sm text-charcoal truncate">{s.name}</div>
                    <div className="text-xs text-muted-foreground">{s.mutualCount} mutual</div>
                  </div>
                  <Button
                    size="sm"
                    variant="ghost"
