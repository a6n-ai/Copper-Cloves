...
function computeHexGeometry(diameter: number, stroke: number, pct: number) {
  ...
  return { pointsStr, perim, dashOffset };
}

function buildFaceBg(color: string) {
  return `radial-gradient(circle at 30% 22%, rgba(255,255,255,0.8), rgba(255,255,255,0) 44%), conic-gradient(from 215deg, color-mix(in oklab, ${color} 44%, white), color-mix(in oklab, ${color} 88%, black) 22%, ${color} 48%, color-mix(in oklab, ${color} 88%, black) 72%, color-mix(in oklab, ${color} 44%, white))`;
}
...
// Earned-only highlight sheen + optional idle shimmer sweep.
function FaceOverlays({ earned, shimmer, reduce, px, py, hovering }: Readonly<FaceOverlaysProps>) {
    ...
    <>
      <div
        ...
        style={{
          ...
          transform: `translate(${px * 26}%, ${py * 26}%)`,
          mixBlendMode: "screen",
...
function MedalFace({
  ...
  py,
  hovering,
}: Readonly<MedalFaceProps>) {
  const progressStroke = reduce ? (
    <polygon
      points={pointsStr}
      fill="none"
      stroke={color}
  ...
  return (
    <>
      ...
      <FaceOverlays earned={earned} shimmer={shimmer} reduce={reduce} px={px} py={py} hovering={hovering} />
    </>
  );
}
...
// Earned flip-card: front hexagon ↔ back hexagon with the badge name.
function FlipCard({ diameter, faceBg, flipLabel, face }: Readonly<FlipCardProps>) {
...
export function Medal({
  ...
  const { pointsStr, perim, dashOffset } = computeHexGeometry(diameter, stroke, pct);
  const faceBg = buildFaceBg(color);
  ...
  if (flippable) {
    return <FlipCard diameter={diameter} faceBg={faceBg} flipLabel={flipLabel as string} face={face} />;
  }
