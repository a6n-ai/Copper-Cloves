...
const PaymentsInSection = dynamic(
  () => import("@/components/admin/dashboard-tabs/PaymentsInSection").then((m) => m.PaymentsInSection),
  { ssr: false, loading: () => <SectionLoadingSkeleton /> },
);
...
export default function AdminManualEntries() {
  const [activeTab, setActiveTab] = useTabQuery(
    MANUAL_TABS.map((t) => t.v),
    "overview",
  );

  return (
    <>
      <SEO title="Manual Entries - Admin" description="Offline money in and out — payments, costs, and instructor payouts" />
        ...
        <main className="min-h-screen">
          <div className="max-w-7xl mx-auto p-6 lg:p-8 space-y-8">
            <AdminPageHeader
              ...
              subtitle="Offline money in and out — non-Razorpay payments, costs, and instructor payouts"
            />
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
              ...
              <TabsContent value="money_in" className="space-y-6">
                <PaymentsInSection />
              </TabsContent>
