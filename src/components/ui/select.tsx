...
function Select({
  ...
  onValueChange,
  ...props
}: React.ComponentPropsWithoutRef<typeof SelectPrimitive.Root> & ActivityExtras) {
  return (
    <SelectPrimitive.Root
      ...
      onValueChange={(value) => {
        if (!disableActivityTracking && value != null && value !== "") {
          logUserSelection({
            control: "radix_select",
            label: activityLabel ?? props.name,
...
>(({ className, children, position = "popper", ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ref={ref}
      className={cn(
        "relative z-50 max-h-96 min-w-32 overflow-hidden rounded-md border bg-popover/95 backdrop-blur-xl text-popover-foreground shadow-md data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95",
        position === "popper" &&
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      "relative flex w-full cursor-default select-none items-center rounded-sm py-1.5 pl-8 pr-2 text-sm outline-hidden transition-colors duration-200 ease-out focus:bg-sage/10 focus:text-sage hover:bg-sage/10 hover:text-sage [&>svg]:mr-2 [&>svg]:h-4 [&>svg]:w-4 [&>svg]:shrink-0 [&>svg]:text-charcoal/40 data-disabled:pointer-events-none data-disabled:opacity-50",
      className
    )}
    {...props}
  >
